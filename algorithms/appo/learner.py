...
def _build_pack_info_from_dones(
    ...
    unique_lengths = torch.unique_consecutive(cpu_lengths)[0]
...
class LearnerWorker:
    ...
    def _broadcast_model_weights(self):
        ...
        model_state = (policy_version, {k: v.to(device='cpu', non_blocking=True) for k, v in  state_dict.items()})
    ...
    def _mark_rollout_buffer_free(self, rollout):
        r = rollout
        self.traj_tensors_available[r.worker_idx, r.split_idx][
            r.env_idx, r.agent_idx, r.traj_buffer_idx
        ] = 1
        ...
        for epoch in range(self.cfg.ppo_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time("bptt"):
                    if self.cfg.use_rnn:
                        core_output_seq, rnn_states = self.actor_critic.forward_core(
                ...
                with timing.add_time("losses"):
                    policy_loss = self._policy_loss(
                        ratio, adv, clip_ratio_low, clip_ratio_high
                    )
                    ...
                    value_loss = self._value_loss(
                        values, old_values, targets, clip_value
                    )
                    ...
                    if (
                        ...
                        or abs(to_scalar(entropy_loss)) > high_loss
                    ):
                        log.warning(
    ...
    def _extract_rollouts(self, data):
        data = AttrDict(data)
        worker_idx, split_idx, traj_buffer_idx = (
            data.worker_idx,
            data.split_idx,
            data.traj_buffer_idx,
        )
        ...
        while not self.terminate:
            ...
            if self._accumulated_too_much_experience(rollouts):
                ...
                with self.resume_experience_collection_cv:
                    log.debug(
                        "Learner %d is resuming experience collection!", self.policy_id
                    )
